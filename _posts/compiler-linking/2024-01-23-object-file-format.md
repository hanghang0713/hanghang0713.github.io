...
### 符号修饰与函数签名
为了区分同名的函数，支持复杂的语言特性，后续发明了 `符号修饰 （Name Decoration）` 或者 `符号改编 (Name Manging) ` 的机制
#### C++ 符号修饰
```C++
int func(int);
float  func(float);
...
上面的代码中有6个同名函数 `func`。为了区分他们，我们引入 `函数签名 (Function Signature)` 的概念。
它包含了一个函数的信息：
...
编译器和链接器在处理符号时，它们使用某种名称修饰的方法，使得每个函数签名对应一个 **修饰后的名称**。

### extern "C"
C++ 为了与 C 兼容，在符号的管理上， 可以使用 `extern "C"` 来声明或定义一个 C 的符号。
`extern "C"` 声明的代码, C++ 的符号修饰机制将不再生效。

### 弱符号与强符号
我们经常碰到一种情况就是符号重复定义，多个目标文件中含有相同名字全局符号的定义。
一般编译器默认函数和初始化的全局变量为`强符号`，未初始化的全局变量为`弱符号`。
...
上面的代码中，`weak` `weak2` 就是弱符号， strong 和 main 就是强符号。`ext` 既不是弱符号也不是强符号，是一个外部变量的引用。

- 规则 1: 不允许强符号被多次定义
- 规则 2：如果一个符号在某个目标文件中是强符号，其他文件中是弱符号，那么选择强符号
- 规则 3：如果一个符号在所有文件中都是弱符号，选择占用空间最大的一个。（因此尽量不要使用不同类型的弱符号，难以定位问题）     

**弱引用与强引用**
对外部文件的符号引用在目标文件被最终链接成可执行文件时，需要被正确决议。这种被称为强引用。
相对应的是弱引用。符号未定义时，链接器对弱引用不会报错，而是默认为 0.

> 弱符号和弱引用对库非常有用，库中定义的弱符号可以被用户自定义的强符号覆盖。
程序可以对某些扩展功能模块的引用定义为弱引用，这使得程序的功能更加容易剪裁和组合
